...
@section('page_title', 'Редактирование предметов - '.$s->name. ' ('.$s->my_class->name.')')
@section('content')

<div class="card">
    <div class="card-header header-elements-inline">
        <h6 class="card-title">Редактирование предметов - {{$s->my_class->name }}</h6>
        {!! Qs::getPanelOptions() !!}
    </div>
            ...
            <div class="col-md-6">
                <form class="ajax-update-h" method="post" action="{{ route('subjects.update', $s->id) }}">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-9">
                            <input name="name" value="{{ $s->name }}" required type="text" class="form-control" placeholder="Название предмета">
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-9">
                            <input name="slug" value="{{ $s->slug }}" type="text" class="form-control" placeholder="">
                        </div>
                    </div>
                    ...
                    <div class="text-right">
                        <button type="submit" class="btn btn-primary">Изменить данные<i class="icon-paperplane ml-2"></i></button>
                    </div>
                </form>
            </div>
        </div>
