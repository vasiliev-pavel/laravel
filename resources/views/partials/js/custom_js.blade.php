...
//        Update Values
    ...
    function submitForm(form, formType){
        ...
        req.fail(function(e){
           ...
           if(e.status == 500){
               displayAjaxErr([e.status + ' ' + e.statusText + ' Пожалуйста, проверьте наличие дубликатов записей или свяжитесь с администратором школы / ИТ-персоналом'])
           }
            if(e.status == 404){
               displayAjaxErr([e.status + ' ' + e.statusText + ' - Запрошенный ресурс или запись не найдены'])
           }
            enableBtn(btn);
            return e.status;
        });
